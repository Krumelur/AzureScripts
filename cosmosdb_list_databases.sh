...
# This is not a full base64 encoding but instead only changes the characters we may see: = -> %3d, & -> %26
urlEncodedAuthString=$(echo -n "$authString" | sed 's/=/%3d/g' | sed 's/&/%26/g')
